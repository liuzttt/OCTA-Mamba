...
########        
        for fname in tqdm(filenames, desc="Processing"):
            ...
            ### décommenter ce block si vous voulez ajouter la légende:
            # full_overlay = add_legend(full_overlay)
            # outline_overlay = add_legend(outline_overlay)
 ...
 ###########       
        ### décommenter si vous voulez Sauvegarde les scores Dice
        # if dice_scores:
        #     dice_scores = np.array(dice_scores)
        #     print(f"Dice: Mean={dice_scores.mean():.4f}, Std={dice_scores.std():.4f}")
        #     np.save(
        #         os.path.join(overlay_output_path, f"dice_{model_name}_{dataset_name}.npy"),
        #         dice_scores
        #     )
